	...
	********************************/
	...
	var openOldImageDialog = function(title,onInsert){
		...
		$('#sds_thumb').remove();
			...
			$.ajax({
				url: 'index.php?route=common/filemanager&token=' + getURLVar('token') + '&target=sdstest_target&thumb=sdstest_thumb&directory=revslider_media_folder',
				dataType: 'html',
				success: function(html){
					$('body').append('<input type="hidden" id="sds_target"><div id="sds_thumb" style=""><img src=""></div><div id="modal-image" class="modal">' + html + '</div>');
					$('#modal-image').modal('show');
				},
				complete: function(){
					$('a.thumbnail').on('click', function(e){
						e.preventDefault();
						var oni = $(this).attr('href');
						onInsert(oni);
	...
	t.d_none_rev_by_id = function(id){
		$('#'+id).hide();
	}
...
//end custom upload sds
//start custom upload sds
function slide_image_upload(field){
	$('#dialog').remove();
	$('body').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?route=common/filemanager&token='+sds_rev_oc_token+'&field=' + encodeURIComponent(field) + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
	$('#dialog').dialog({
		title: 'Image Uploader',
			...
			if ($('#' + field).attr('value')) {
				$.ajax({
					...
					success: function(data) {
						...
						$("#divbgholder").css("background-image","url("+img_src+")");
						$("#radio_back_image").attr('checked', 'checked');
						$("#radio_back_image").click();
						if($('input[name="kenburn_effect"]:checked').val() == 'on'){
							$('input[name="kb_start_fit"]').change();
						}
					}
				});
			}
...
$(document).ready(function(){
	UniteAdminRev.d_none_rev_by_id("jquery_noconflict_row");
	UniteAdminRev.d_none_rev_by_id("js_to_body_row");
});
