<?php
	...
	class RevSlide extends UniteElementsBaseRev{
		...
		public function initByPostData($postData, RevSlide $slideTemplate, $sliderID){
			...
			if(!empty($postID))
				$this->setImageByImageID($postID,$sliderID);
		...
		private function setLayersByPostData($postData,$sliderID){
			...
			$description_short = strip_tags($postData["description"]);

			$description_short = substr($description_short, $excerpt_limit);
			//$description_short = htmlspecialchars_decode($description_short);

			$link = $postData["link"];

///////////////////////////////////////////////////////
			if(strpos(HTTP_SERVER, 'admin/') !== false){
				$rsacobj = get_instance('ControllerModulerevslideropencart');
			}else{
				$rsacobj = get_instance('ControllerModulerevslideroutput');
			}
			$productprice = $rsacobj->price_format($postData["price"]);

////////////////////////////////////////////////////////

			$date_add = $postData["date_added"];

            $date_upd = $postData["date_modified"];
		...
		public function updateSlideFromData($data, $slideSettings){
			...
			if(gettype($layers) == "string"){
				...
				$layersStrip_org = htmlspecialchars_decode(trim($layersStrip));
				$layersDecoded = json_decode($layersStrip_org);
				if(empty($layersDecoded)){
					$layers_org = htmlspecialchars_decode(trim($layers));
					$layersDecoded = json_decode($layers_org);
				}
