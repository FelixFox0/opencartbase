<?php
	...
	class RevOperations extends UniteElementsBaseRev{
		...
		public static function getFullCustomAnimations(){
			...
			if(!empty($result)){
				foreach($result as $key => $value){
					...
					$value_params = htmlspecialchars_decode(trim($value['params']));
					$customAnimations[$key]['params'] = json_decode(str_replace("'", '"', $value_params), true);
				}
			}
		...
		public static function getCustomAnimationByHandle($handle){
			...
			if(!empty($result)){
				$result_params = htmlspecialchars_decode(trim($result[0]['params']));
			 return json_decode(str_replace("'", '"', $result_params), true);
			}
		...
		public static function getFullCustomAnimationByID($id){
			...
			if(!empty($result)){
				...
				$result_params = htmlspecialchars_decode(trim($result[0]['params']));
				$customAnimations['params'] = json_decode(str_replace("'", '"', $result_params), true);
				return $customAnimations;
			}
		...
		public function insertCaptionsContentData($content){
			...
			if(isset($content['handle'])) {	
				...
				//$arrInsert["params"] = stripslashes(json_encode($content['params']));
                                
				$arrInsert["params"] = json_encode($this->rev_clear_quote(@$content['params']));

                                $arrInsert["hover"] = json_encode($this->rev_clear_quote(@$content['hover']));

                                $arrInsert["settings"] = json_encode($this->rev_clear_quote(@$content['settings']));

                                
                                if (get_magic_quotes_gpc()) {
                                
                                    $arrUpdate["params"] = stripslashes(json_encode(@$content['params']));

                                    $arrUpdate["hover"] = stripslashes(json_encode(@$content['hover']));

                                    $arrUpdate["settings"] = stripslashes(json_encode(@$content['settings']));
		 ...
		 */
				public function updateCaptionsContentData($content){
					...
					if(isset($content['handle'])) { 	
						...
						if(isset($content["params"])){
							$arrUpdate["params"] = json_encode($this->rev_clear_quote(@$content['params']));
						}
						if(isset($content["hover"])){
							$arrUpdate["hover"] = json_encode($this->rev_clear_quote(@$content['hover']));
						}
						if(isset($content["settings"])){
							$arrUpdate["settings"] = json_encode($this->rev_clear_quote(@$content['settings']));
						}
				...
				public function rev_clear_quote($content){                    
...
// end add font 
					...
					?>
						...
						<?php
							...
							<link rel='stylesheet' href='<?php echo $urlPlugin?>css/captions.css?rev=<?php echo GlobalsRevSlider::SLIDER_REVISION; ?>' type='text/css' media='all' />
