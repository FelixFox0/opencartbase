...
//				self::$urlAdmin .= "/";
		...
		public static function isDBTableExists($tableName){
                        ...
                        if(class_exists('ControllerModulerevslideropencart'))                        
                            ...
                            $wpdb = ControllerModulerevslideropencart::$wpdb;
        ...
        public static function import_media_img($file_url, $folder, $filename){
                    ...
                    if(!empty($mysqli) && is_numeric($imgid)){
                        ...
                        foreach($fsizes as $sizew=>$sizeh){
                            ...
                            $res_img = new SdsRevToolImage();
                            if(++$count > 1){
                                $res_img->resize($folder.$filename, $sizew, $sizeh,$folder.$newfilename);
                            }else{
                                $res_img->resize($folder.$filename, $sizew, $sizeh,$folder.$newfilename);
                            }
                        }
                        return array("id" => $imgid, "path" => 'image/catalog/revslider_media_folder/'.$filename);
                    }
                }
		...
		public static function _import_media($file_url, $folder_name) { //old name import_media
   ...
   //          {
		...
		public static function getPostsByIDs($strIDs){
			...
			$i = 0;
			foreach($arr as $ar){
				$product = sdsconfig::getProduct($ar);
				$arrPosts[$i] = $product;
				$i++;
			}
			return($arrPosts);
		...
		//start get opencart product Category and description
		...
		public static function GetRevPostDataArray($catIDs,$sortBy = self::SORTBY_ID,$direction = self::ORDER_DIRECTION_DESC,$numPosts=-1,$postTypes="any",$taxonomies="category",$arrAddition = array())
		    {
		    	...
		    	if(isset($categoriesid) && !empty($categoriesid)){
		    		foreach($categoriesid as $catid){
			    		...
			    		if(isset($results_temp) && !empty($results_temp)){
		...
		public static function getCMSPages(){
            $pages = sdsconfig::getallinformations();
            $npages = array();
            if(!empty($pages)){
                foreach($pages as $page){
                    $npages['CMS'.$page['information_id']] = htmlspecialchars_decode($page['title']);
                }
            }
            return $npages;
        }
        public static function getPrestaProdChildCat($cat_id){
        		$cats = sdsconfig::getCategories($cat_id);
        		if(!empty($cats)){
        			foreach($cats as $cat){
