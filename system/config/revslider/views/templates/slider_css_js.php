...
$custom_js = '';
if(!empty($sliderID)){
	if(isset($arrFieldsParams['custom_css'])){
		$custom_css = stripslashes($arrFieldsParams['custom_css']);
	}else{
		$custom_css = '';
	}
	if(isset($arrFieldsParams['custom_javascript'])){
...
?>
	<div id="toolbox_wrapper" class="toolbox_wrapper postbox unite-postbox rs-cm-refresh">
		<h3 class="box_closed tp-accordion tpa-closed"><div class="postbox-arrow"></div><i style="float:left;margin-top:4px;font-size:14px;" class="eg-icon-magic"></i><span><?php echo ControllerModulerevslideropencart::$lang_var

['CSS_JavaScript'] ?></span></h3>
